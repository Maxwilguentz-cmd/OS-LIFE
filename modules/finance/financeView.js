...
export function renderFinanceView(state) {
  ...
  // Rekipere diksyonè tradiksyon an selon lang aktyèl la
  const currentLang = getCurrentLanguage();
  const dict = TRANSLATIONS[currentLang].financePage;
  ...
  // Jenere opsyon pou seleksyon kategori yo ak tradiksyon yo
  const categoryOptions = ALLOWED_CATEGORIES.map(cat => {
    const label = dict.categoryLabels[cat] || (cat.charAt(0).toUpperCase() + cat.slice(1));
    ...
    <div class="finance-container" style="display: flex; flex-direction: column; gap: 20px; padding: 10px;">
      ...
      <div class="finance-summary" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 15px;">
        <div class="card glass" style="padding: 15px; border-radius: 8px; border: 1px solid rgba(255,255,255,0.1);">
          <h4 style="margin: 0; font-size: 14px; opacity: 0.8;">${dict.balanceLabel}</h4>
          <h2 style="margin: 5px 0 0 0; color: ${reports.balance >= 0 ? '#4caf50' : '#f44336'};">${reports.balance} HTG</h2>
        </div>
        <div class="card glass" style="padding: 15px; border-radius: 8px; border: 1px solid rgba(255,255,255,0.1);">
          <h4 style="margin: 0; font-size: 14px; opacity: 0.8;">${dict.totalIncomeLabel}</h4>
          <h2 style="margin: 5px 0 0 0; color: #4caf50;">+${reports.totalIncome} HTG</h2>
        </div>
        <div class="card glass" style="padding: 15px; border-radius: 8px; border: 1px solid rgba(255,255,255,0.1);">
          <h4 style="margin: 0; font-size: 14px; opacity: 0.8;">${dict.totalExpenseLabel}</h4>
      ...
      <div class="finance-grid" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap: 20px;">
        ...
        <div class="card glass" style="padding: 20px; border-radius: 8px; border: 1px solid rgba(255,255,255,0.1);">
          <h3>${dict.savingsTitle}</h3>
          <div style="margin-bottom: 15px;">
            <p style="margin: 5px 0;"><strong>${dict.goalLabel}</strong> ${savingsData.goalName || 'General Savings'}</p>
            <p style="margin: 5px 0;"><strong>${dict.progressLabel}</strong> ${savingsData.current} / ${savingsData.goal} HTG (${reports.savingsProgressPercentage}%)</p>
          ...
          <form id="savingsForm" style="display: flex; flex-direction: column; gap: 10px;">
            <div style="display: flex; gap: 10px;">
              <input type="number" id="savingsCurrentInput" placeholder="${dict.currentPlaceholder}" value="${savingsData.current}" required style="flex: 1; padding: 8px; border-radius: 4px; border: 1px solid rgba(255,255,255,0.2); background: rgba(0,0,0,0.2); color: white;" />
              <button type="submit" class="btn-primary" style="padding: 8px 15px; cursor: pointer;">${dict.updateBtn}</button>
            </div>
          </form>
        </div>
        ...
        <!-- FÒM TRANZAKSYON -->
        <div class="card glass" style="padding: 20px; border-radius: 8px; border: 1px solid rgba(255,255,255,0.1);">
          <h3>${dict.newTxTitle}</h3>
          <form id="transactionForm" style="display: flex; flex-direction: column; gap: 12px;">
            <div style="display: flex; gap: 10px;">
              <label style="flex: 1; color: white;"><input type="radio" name="txType" value="income" checked /> ${dict.incomeLabel}</label>
              <label style="flex: 1; color: white;"><input type="radio" name="txType" value="expense" /> ${dict.expenseLabel}</label>
            </div>
            
            <input type="number" id="txAmount" placeholder="${dict.amountPlaceholder}" required style="padding: 8px; border-radius: 4px; border: 1px solid rgba(255,255,255,0.2); background: rgba(0,0,0,0.2); color: white;" />
            ...
            <select id="txCategory" required style="padding: 8px; border-radius: 4px; border: 1px solid rgba(255,255,255,0.2); background: rgba(0,0,0,0.2); color: white;">
              <option value="" disabled selected>${dict.categoryPlaceholder}</option>
              ${categoryOptions}
            </select>
            
            <input type="text" id="txDescription" placeholder="${dict.descriptionPlaceholder}" required style="padding: 8px; border-radius: 4px; border: 1px solid rgba(255,255,255,0.2); background: rgba(0,0,0,0.2); color: white;" />
            
            <button type="submit" class="btn-primary" style="padding: 10px; cursor: pointer; border-radius: 4px;">${dict.addTxBtn}</button>
          </form>
        </div>
      ...
      <div class="card glass" style="padding: 20px; border-radius: 8px; border: 1px solid rgba(255,255,255,0.1); margin-top: 10px;">
        ...
        <div id="transactionsList" style="max-height: 300px; overflow-y: auto; display: flex; flex-direction: column; gap: 10px; margin-top: 10px;">
          ${sortedTransactions.length === 0 ? `<p style="opacity: 0.6; text-align: center;">${dict.emptyTx}</p>` : ''}
          ${sortedTransactions.map(tx => {
            const categoryLabel = dict.categoryLabels[tx.category] || tx.category;
            return `
              <div class="transaction-item" data-id="${tx.id}" style="display: flex; justify-content: space-between; align-items: center; padding: 10px; background: rgba(255,255,255,0.05); border-radius: 6px; border-left: 4px solid ${tx.type === 'income' ? '#4caf50' : '#f44336'};">
                <div>
                  ...
                  <span style="font-size: 12px; opacity: 0.6; text-transform: uppercase;">${categoryLabel}</span>
                </div>
