...
export function renderAll(state) {
  ...
  const kounye a = new Date();
  const minitPase = (kounye a.getHours() * 60) + kounye a.getMinutes();
  ...
  if (arcTime) {
    arcTime.textContent = kounye a.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', hour12: false });
  }
  if (arcFill) {
  ...
  if (missionList && state.missions) {
    missionList.innerHTML = state.missions.map(m => `
      <li class="mission-item ${m.is-done || m.done ? 'is-done' : ''}" data-id="${m.id}">
  ...
  if (weeklyBars && state.weeklyStats?.dailyData) {
    ...
    const jodi endeks = (new Date().getDay() + 6) % 7; // Konvèti pou Lendi kòmanse kòm 0
    
    weeklyBars.innerHTML = state.weeklyStats.dailyData.map((dataVal, i) => `
      <div class="bar-col">
        <div class="bar-track">
          <div class="bar-fill ${i === jodi_endeks ? 'is-today' : ''}" style="height: ${dataVal}%"></div>
        </div>
        <span class="bar-day ${i === jodi_endeks ? 'is-today' : ''}">${jouYo[i]}</span>
      </div>
    `).join('');
  }
}
